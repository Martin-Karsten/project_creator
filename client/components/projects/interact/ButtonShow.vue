...
export default {
    ...
    methods: {
        ...
        formatFunction(arr){
            arr.forEach(x => {
                this.$store.dispatch("LayoutItems/Interact/Button/formatActionShow", {currentItem: x, event: this.currentItem.sidebarColor})
            })
        },
        borderStyleFunction(arr){
            arr.forEach(x => {
                this.$store.dispatch("LayoutItems/Interact/Button/borderStyleActionShow", {currentItem: x, event: this.currentItem.sidebarColor})
            })
        },
        hideFunction(arr){
            arr.forEach(x => {
                this.$store.dispatch("LayoutItems/Interact/Button/hideActionShow", {currentItem: x, event: 'is-hidden'})
            })
        }
    }
}
