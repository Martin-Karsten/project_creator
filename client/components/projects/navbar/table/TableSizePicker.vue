...
export default {
  data() {
    return {
      columns: Array.apply(null, new Array(100)).map(function() {
        return false
      }),
      gridY: 0,
      gridX: 0
  ...
  methods: {
    selectColumn(index) {
      let y = 0
      if (index >= 10) y = (index + "").charAt(0)
      else y = 0

      let x = 0
      if (index < 10) x = (index + "").charAt(0)
      else x = (index + "").charAt(1)
      ...
      this.$store.dispatch("LayoutItems/Table/createTable", {
        layoutId: 2,
        rows: this.gridY,
        columns: this.gridX
      })
    ...
    resetPicker() {
      this.gridY = ""
      this.gridX = ""
      this.columns = Array.apply(null, new Array(100)).map(function() {
        return false
      })
    }
  }
