    ...
    if (state.index == 0) {
      switch (payload.name) {
        case "sidebarTextfield":
          ...
          state.sidebar[0] =
            rootGetters["SidebarTextfield/getSidebarTextfield"][0]
          state.index = payload.id
          break
      ...
      switch (payload.name) {
        case "sidebarTextfield":
          ...
          state.sidebar[state.sidebar.length - 1] =
            rootGetters["SidebarTextfield/getSidebarTextfield"][
              rootGetters["SidebarTextfield/getSidebarTextfield"].length - 1
            ]
          state.index = payload.id
          break
        ...
        case "sidebarImage":
          ...
          commit("ADD_COMPONENT")
          state.sidebar[state.sidebar.length - 1] =
            rootGetters["SidebarImage/getSidebarImage"][
              rootGetters["SidebarImage/getSidebarImage"].length - 1
            ]
          console.log(state.sidebar, state.index)
          break
        case "sidebarCharts":
