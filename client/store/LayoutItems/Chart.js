import Vue from 'vue'
import falttenObject from '../../Helper/Project/LayoutHelper.js';
...
export const mutations = {
    ...
    ADD_LINE_CHART(state, payload) {
        ...
        let obj = {[payload.layoutId] : {id: id, project_id: state.projectId, name: 'chart',
        background_color: 'none', border_color: 'black', border_style: 'solid', animations:{}, itemName: 'charts',
        chart_settings:{id: id, chart_id: id, 
    ...
    UPDATE_CHART(state, payload) {
        Object.entries(payload).forEach(
            ([key, value]) => {
                if(state.charts[1].chart_settings[key] != null && Object.keys(state.charts[1].chart_settings[key]).length === 0)
                    state.charts[1].chart_settings[key] = { ...state.charts[1].chart_settings[key], ...payload[key]}
            }
        )
    },
    ...
    UPDATE_YAXIS_TITLE_SIZE(state, payload) {
        payload.currentItem.chart_settings.yAxis.nameTextStyle.fontSize = payload.fontSize
    },
    UPDATE_XAXIS_TITLE_SIZE(state, payload) {
        payload.currentItem.chart_settings.xAxis.nameTextStyle.fontSize = payload.fontSize
    },
    UPDATE_YAXIS_VALUE(state, payload) {
        payload.currentItem.chart_settings.series[0].data.splice(payload.index, 1, payload.value)
    },
    UPDATE_XAXIS_VALUE(state, payload) {
        payload.currentItem.chart_settings.xAxis.data.splice(payload.index, 1, payload.value)
    },
    ADD_SERIES(state, payload) {
        state.lineColorIndex++
        if(state.lineColorIndex === 10)
            state.lineColorIndex = 0
    ...
    UPDATE_SERIES_COLOR(state, payload) {
        payload.currentItem.chart_settings.series[payload.index].itemStyle.color = payload.color
    },
...
export const actions = {
    ...
    addLineChart({state, commit}, payload){
        ...
        commit('Layout/ADD_CHART', {layoutId: payload.layoutId, id: id}, {root: true})
    },
